...
def data_loader(q, ):
    for start in range(0, len(ids_test), batch_size):
        print('generating masks for testing data....',str(start),'/',str(len(ids_test)),'{}% finished'.format(start/len(ids_test)))
        x_batch = []
...
def predictor(q, ):
    for i in range(0, len(ids_test), batch_size):
        ...
        for pred in preds:
            ...
            mask = prob > threshold
            cv2.imwrite('optimized_test_masks_1/'+ids_test[i]+'_mask.png',mask*255)
